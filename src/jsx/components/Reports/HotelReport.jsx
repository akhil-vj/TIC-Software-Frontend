...
const HotelReport = () => {
    const { loading, hotelReports } = useReportData();
    ...
    const filtered = hotelReports
        .filter(h => h.hotelName.toLowerCase().includes(search.toLowerCase()) || h.destination.toLowerCase().includes(search.toLowerCase()))
        .sort((a, b) => sortDesc ? (b[sortCol] || 0) - (a[sortCol] || 0) : (a[sortCol] || 0) - (b[sortCol] || 0));

    const totals = filtered.reduce((acc, h) => ({
        bookings: acc.bookings + h.bookings,
        nights: acc.nights + h.nights,
    ...
    const handleExcelExport = () => {
        exportToExcel(
            ...
            (h) => [h.hotelName, h.destination, h.bookings, h.nights, h.revenue, h.rating, h.occupancy + "%"]
        );
    };
    ...
    const renderStars = (rating) => {
        ...
        return "★".repeat(r) + "☆".repeat(Math.max(5 - r, 0));
    };
                ...
                <div className="col-12">
                    <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                        ...
                        <button className="btn btn-success btn-sm" onClick={handleExcelExport} disabled={loading}>
                            <i className="las la-file-excel me-1"></i>Download Hotel Report
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="row">
                <div className="col-12">
                    <div className="excel-tabs">
                        {sheets.map((s, i) => (
                            <Link key={i} to={s.path} className={`excel-tab ${s.path === "/reports/hotels" ? "active" : ""}`}>
                    ...
                    <div className="card border-0 shadow-sm" style={{ borderRadius: "0 0 8px 8px" }}>
                        <div className="card-body p-0">
                            <div className="d-flex justify-content-between align-items-center p-3 border-bottom">
                                <span className="text-muted fs-13">Hotel Report — Partner hotel bookings and revenue</span>
                                <input type="text" className="form-control form-control-sm" style={{ maxWidth: 220 }} placeholder="Search hotel or destination..." value={search} onChange={e => setSearch(e.target.value)} />
                            </div>
                            {loading ? (
                                    ...
                                    <table className="report-table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th className="num" onClick={() => toggleSort("nights")}>Room Nights{sortArrow("nights")}</th>
                                                <th className="num" onClick={() => toggleSort("revenue")}>Revenue{sortArrow("revenue")}</th>
                                                <th className="num" onClick={() => toggleSort("rating")}>Rating{sortArrow("rating")}</th>
                                                <th className="num" onClick={() => toggleSort("occupancy")}>Occupancy{sortArrow("occupancy")}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {filtered.map((h, i) => (
                                                <tr key={i}>
                                                    ...
                                                    <td style={{ fontWeight: 500 }}>{h.hotelName}</td>
                                                    <td>{h.destination}</td>
                                                    <td className="num">{h.bookings}</td>
                                                    <td className="num">{h.nights} Nights</td>
                                                    <td className="num">{fmt(h.revenue)}</td>
                                                    <td className="num" style={{ color: "#f59e0b", letterSpacing: 1 }}>{renderStars(h.rating)}</td>
                                                    <td className="num">{h.occupancy}%</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                        <tfoot>
                                            <tr>
                                                ...
                                                <td className="num">{totals.nights} Nights</td>
