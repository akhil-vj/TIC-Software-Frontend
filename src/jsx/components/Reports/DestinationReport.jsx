...
const DestinationReport = () => {
    const { loading, destinationSales } = useReportData();
    ...
    const filtered = destinationSales
        .filter(d => d.destination.toLowerCase().includes(search.toLowerCase()))
    ...
    const handleExcelExport = () => {
        exportToExcel(
            ...
            (d) => [d.destination, d.bookings, d.pax, d.revenue, d.profit]
        );
    };
                ...
                <div className="col-12">
                    <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                        ...
                        <button className="btn btn-success btn-sm" onClick={handleExcelExport} disabled={loading}>
                            <i className="las la-file-excel me-1"></i>Download Destination Report
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="row">
                <div className="col-12">
                    <div className="excel-tabs">
                        {sheets.map((s, i) => (
                            <Link key={i} to={s.path} className={`excel-tab ${s.path === "/reports/destination" ? "active" : ""}`}>
                    ...
                    <div className="card border-0 shadow-sm" style={{ borderRadius: "0 0 8px 8px" }}>
                        <div className="card-body p-0">
                            <div className="d-flex justify-content-between align-items-center p-3 border-bottom">
                                <span className="text-muted fs-13">Destination Sales Report — Revenue by destination from enquiry data</span>
                                <input type="text" className="form-control form-control-sm" style={{ maxWidth: 220 }} placeholder="Search destination..." value={search} onChange={e => setSearch(e.target.value)} />
                            </div>
                            {loading ? (
                                ...
                                <div className="table-responsive">
                                    <table className="report-table">
                                        <thead>
                                            <tr>
                                                <th>#</th>
                                                <th onClick={() => toggleSort("destination")}>Destination{sortArrow("destination")}</th>
                                                <th className="num" onClick={() => toggleSort("bookings")}>Bookings{sortArrow("bookings")}</th>
                                                <th className="num" onClick={() => toggleSort("pax")}>Pax{sortArrow("pax")}</th>
                                        ...
                                        <tbody>
                                            {filtered.map((d, i) => (
                                                <tr key={i}>
                                                    ...
                                                    <td className="num" style={{ color: d.profit >= 0 ? "#198754" : "#dc3545" }}>{fmt(d.profit)}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                        <tfoot>
